...
import de.robv.android.zposed.XC_MethodHook;
import de.robv.android.zposed.XC_MethodReplacement;
import de.robv.android.zposed.ZposedBridge;
import de.robv.android.zposed.ZposedHelpers;
...
public class ParasiticManagerHooker {
    ...
    private static void sendBinderToManager(final ClassLoader classLoader, IBinder binder) {
        try {
            var clazz = ZposedHelpers.findClass("org.lsposed.manager.Constants", classLoader);
            var ok = (boolean) ZposedHelpers.callStaticMethod(clazz, "setBinder",
    ...
    private static void hookForManager(ILSPManagerService managerService) {
        var managerApkHooker = new XC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) {
                ...
                ApplicationInfo appInfo = (ApplicationInfo) ZposedHelpers.getObjectField(bindData, "appInfo");
                ZposedHelpers.setObjectField(bindData, "appInfo", getManagerPkgInfo(appInfo).applicationInfo);
            }
        };
        ZposedHelpers.findAndHookMethod(ActivityThread.class,
                "handleBindApplication",
                "android.app.ActivityThread$AppBindData",
                managerApkHooker);

        var unhooks = new XC_MethodHook.Unhook[]{null};
        unhooks[0] = ZposedHelpers.findAndHookMethod(
                LoadedApk.class, "getClassLoader", new XC_MethodHook() {
                    @Override
                    protected void afterHookedMethod(MethodHookParam param) {
                        var pkgInfo = getManagerPkgInfo(null);
                        if (pkgInfo != null && ZposedHelpers.getObjectField(param.thisObject, "mApplicationInfo") == pkgInfo.applicationInfo) {
        ...
        var activityClientRecordClass = ZposedHelpers.findClass("android.app.ActivityThread$ActivityClientRecord", ActivityThread.class.getClassLoader());
            ...
            protected void afterHookedMethod(MethodHookParam param) {
                for (var i = 0; i < param.args.length && activityClientRecordClass.isInstance(param.thisObject); ++i) {
                    if (param.args[i] instanceof ActivityInfo) {
                        ...
                        Hookers.logD("loading state of " + aInfo.name);
                        states.computeIfPresent(aInfo.name, (k, v) -> {
                            ZposedHelpers.setObjectField(param.thisObject, "state", v);
                            return v;
                        });
                        persistentStates.computeIfPresent(aInfo.name, (k, v) -> {
                            ZposedHelpers.setObjectField(param.thisObject, "persistentState", v);
                            return v;
                        });
                    }
                }
        ...
        ZposedBridge.hookAllConstructors(activityClientRecordClass, activityHooker);

        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.O_MR1) {
            ZposedBridge.hookAllMethods(ZposedHelpers.findClass("android.app.ActivityThread$ApplicationThread", ActivityThread.class.getClassLoader()), "scheduleLaunchActivity", activityHooker);
        }

        ZposedBridge.hookAllMethods(ActivityThread.class, "handleReceiver", new XC_MethodReplacement() {
            @Override
        ...
        ZposedBridge.hookAllMethods(ActivityThread.class, "installProvider", new XC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) {
                ...
                if (ctx != null && info != null && pkgInfo != null) {
                    ...
                    if (originalContext == null) {
                        ...
                        ZposedHelpers.setObjectField(originalPkgInfo, "mPackageName", packageName);
                        originalContext = (Context) ZposedHelpers.callStaticMethod(ZposedHelpers.findClass("android.app.ContextImpl", null),
        ...
        ZposedHelpers.findAndHookMethod(ActivityThread.class, "deliverNewIntents", activityClientRecordClass, List.class, new XC_MethodHook() {
            @Override
            protected void beforeHookedMethod(MethodHookParam param) {
        ...
        ZposedHelpers.findAndHookMethod(WebViewFactory.class, "getProvider", new XC_MethodReplacement() {
            @Override
            protected Object replaceHookedMethod(MethodHookParam param) {
                var sProviderInstance = ZposedHelpers.getStaticObjectField(WebViewFactory.class, "sProviderInstance");
                if (sProviderInstance != null) return sProviderInstance;
                //noinspection unchecked
                var providerClass = (Class<WebViewFactoryProvider>) ZposedHelpers.callStaticMethod(WebViewFactory.class, "getProviderClass");
                ...
                try {
                    ...
                    ZposedHelpers.setStaticObjectField(WebViewFactory.class, "sProviderInstance", sProviderInstance);
            ...
            protected void beforeHookedMethod(MethodHookParam param) {
                try {
                    ...
                    if (record instanceof IBinder) {
                        record = ((ArrayMap<?, ?>) ZposedHelpers.getObjectField(param.thisObject, "mActivities")).get(record);
                        if (record == null) return;
                    }
                    ZposedHelpers.callMethod(param.thisObject, Build.VERSION.SDK_INT >= Build.VERSION_CODES.P ? "callActivityOnSaveInstanceState" : "callCallActivityOnSaveInstanceState", record);
                    var state = (Bundle) ZposedHelpers.getObjectField(record, "state");
                    var persistentState = (PersistableBundle) ZposedHelpers.getObjectField(record, "persistentState");
                    var aInfo = (ActivityInfo) ZposedHelpers.getObjectField(record, "activityInfo");
        ...
        ZposedBridge.hookAllMethods(ActivityThread.class, "performStopActivityInner", stateHooker);
        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.O_MR1)
            ZposedHelpers.findAndHookMethod(ActivityThread.class, "performDestroyActivity", IBinder.class, boolean.class, int.class, boolean.class, stateHooker);
    }
