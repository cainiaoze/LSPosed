...
import static de.robv.android.zposed.ZposedHelpers.decrementMethodDepth;
import static de.robv.android.zposed.ZposedHelpers.findAndHookMethod;
import static de.robv.android.zposed.ZposedHelpers.getBooleanField;
import static de.robv.android.zposed.ZposedHelpers.getLongField;
import static de.robv.android.zposed.ZposedHelpers.getObjectField;
import static de.robv.android.zposed.ZposedHelpers.incrementMethodDepth;
...
import de.robv.android.zposed.IZposedHookZygoteInit;
import de.robv.android.zposed.XC_MethodHook;
import de.robv.android.zposed.XC_MethodHook.MethodHookParam;
import de.robv.android.zposed.ZposedBridge;
import de.robv.android.zposed.ZposedBridge.CopyOnWriteSortedSet;
import de.robv.android.zposed.ZposedInit;
import de.robv.android.zposed.callbacks.XC_LayoutInflated;
import de.robv.android.zposed.callbacks.XC_LayoutInflated.LayoutInflatedParam;
import de.robv.android.zposed.callbacks.XCallback;
import zposed.dummy.XResourcesSuperClass;
import zposed.dummy.XTypedArraySuperClass;
...
//			Log.w(ZposedBridge.TAG, "Package name for " + resDir + " had to be retrieved via parser");
	 ...
	 * {@link IZposedHookZygoteInit#initZygote initZygote()} for all apps. As there is no
			...
			if (incrementMethodDepth("getDrawable") == 1) {
				...
				if (replacement instanceof DrawableLoader) {
					try {
						...
						if (result != null)
							return result;
					} catch (Throwable t) { ZposedBridge.log(t); }
				} else if (replacement instanceof Integer) {
			...
			if (callbacks != null) {
				...
				if (value.type == TypedValue.TYPE_STRING) {
					...
					if (components.length == 3)
						...
						ZposedBridge.log("Unexpected resource path \"" + value.string.toString()
								+ "\" for resource id 0x" + Integer.toHexString(id));
				} else {
					ZposedBridge.log(new NotFoundException("Could not find file name for resource id 0x") + Integer.toHexString(id));
				}
	...
	public void getValue(int id, TypedValue outValue, boolean resolveRefs) throws NotFoundException {
		...
		if (replacement instanceof XResForwarder) {
			...
			if (replacement != null) {
				ZposedBridge.log("Replacement of resource ID #0x" + Integer.toHexString(id) + " escaped because of deprecated replacement. Please use XResForwarder instead.");
			}
	...
	private static int translateResId(int id, XResources origRes, Resources repRes) {
		try {
			...
			if (!repResDefined && origResId == 0 && !entryType.equals("id")) {
				ZposedBridge.log(entryType + "/" + entryName + " is neither defined in module nor in original resources");
				return 0;
			}
	...
	private static int translateAttrId(String attrName, XResources origRes) {
		...
		try {
			...
			ZposedBridge.log("Attribute " + attrName + " not found in original resources");
		}
		return origAttrId;
	}
